...
class DeckManager:
    def __init__(self, sit_file="situations.json", ans_file="answers.json", base: Path | None = None):
        # Базой делаем корень проекта: на уровень выше, чем этот файл
    ...
    def get_new_shuffled_answers_deck(self):
        deck = list(dict.fromkeys(self.answers))  # убрать дубли
        random.shuffle(deck)
        return deck


# Читает именно корневые situations.json и answers.json
...
async def send_illustration(bot: Bot, chat_id: int, situation: str, answer: str) -> bool:
    ...
    # 2) Фолбэки при проблемах/лимитах
    if not img:
...
async def _try_gemini_image(prompt: str) -> BytesIO | None:
    """
    Генерация изображения через Gemini 1.5 (Google AI Studio, REST Images).
...
async def _try_nanobanana(prompt: str) -> BytesIO | None:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if not img_url:
                return None
            async with session.get(img_url, timeout=60) as r:
                if r.status != 200:
                    return None
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            for _ in range(40):
                ...
                async with session.get(check + task, headers=headers, timeout=15) as st:
                    js = await st.json()
                    if js.get("done"):
                        ...
                        if not imgs:
                            return None
                        img_url = imgs[0]
                        async with session.get(img_url, timeout=60) as g:
                            if g.status != 200:
                                return None
