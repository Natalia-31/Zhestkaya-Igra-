...
# === ИЗМЕНЕНО: ответ + немедленный добор ровно одной карты, запрет повторного ответа ===
...
async def on_answer(cb: CallbackQuery):
    ...
    # Игрок уже отправил ответ в этом раунде
    ...
    # Снимаем выбранную карту и фиксируем ответ
    card = hand.pop(idx)
    st["answers"][uid] = card
    st["used_answers"].append(card)
    await cb.answer(f"Вы выбрали: {card}")

    # Немедленный добор РОВНО одной карты, чтобы сохранить размер руки
    if not st["main_deck"]:
        full_deck = decks.get_new_shuffled_answers_deck()
    ...
    # Если собраны все ответы — показать их в группе
...
async def on_pick(cb: CallbackQuery):
    ...
    # Заменено: генерация видео вместо изображения
    await video_gen.send_video_illustration(cb.bot, group_chat_id, st["current_situation"], win_ans)

    # Убрано: добор карт здесь больше не нужен, т.к. добираем сразу после ответа
