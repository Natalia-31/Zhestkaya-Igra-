...
async def cmd_start(m: Message):
    # стартовое меню в групповой чат
...
async def ui_new_game(cb: CallbackQuery):
    ...
    await cb.answer()
    # в общий чат убираем кнопки
    await cb.message.edit_reply_markup(reply_markup=None)
    await cb.message.answer(format_info("Игра начата!"))
    # приватно ведущему присылаем кнопку «Присоединиться»
    await cb.bot.send_message(host_id, format_info("Вы ведущий! Ждём участников..."), reply_markup=menu_joinable())
...
async def ui_start_round(cb: CallbackQuery):
    ...
    # ведущему в приват пришлём кнопку «Новый раунд»
    await cb.message.edit_reply_markup(reply_markup=None)
...
async def _join_flow(chat_id: int, user_id: int, user_name: str, bot: Bot, feedback: Message):
    ...
    if user_id not in [p["user_id"] for p in st["players"]]:
        try:
            await bot.send_message(user_id, format_info("Вы присоединились! Ждём, пока ведущий запустит раунд."), reply_markup=None)
        except TelegramBadRequest:
            await feedback.answer(format_error("Невозможно отправить личное сообщение"), reply_markup=None)
            return
...
async def _start_round(bot: Bot, chat_id: int):
    ...
    if not st or len(st["players"]) < 2:
        return await bot.send_message(chat_id, format_error("Нужно минимум 2 игрока"), reply_markup=None)
