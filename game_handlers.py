...
def make_answers_keyboard(hand: List[str]) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"👉 {card}", callback_data=f"ans:{i}")] for i, card in enumerate(hand)
    ])
...
async def cmd_start_round(message: Message):
    ...
    # Генерация
    situation = (await generate_situations_via_openai())[0]
...
async def cb_pick_answer(callback: CallbackQuery):
    ...
    if len(game.answers) >= len(game.player_ids) - 1:
        await show_answers_for_all(callback.message.bot, game.chat.id)
...
async def cb_pick_winner(callback: CallbackQuery):
    ...
    user = callback.from_user
    if game.phase != "choose" or user.id != game.current_host_id():
        return await callback.answer("Нельзя выбрать сейчас.", show_alert=True)
