# handlers/game_handlers.py — версия с генерацией видео Runway после выбора победителя
...
from game_utils import gen  # gen.py в вашем проекте
...
async def _create_game(chat_id: int, host_id: int, host_name: str):
    SESSIONS[chat_id] = {
        ...
        # Хранение URL иллюстрации последнего победителя (для image-to-video консистентности)
...
async def on_answer(cb: CallbackQuery):
    # формат ans:<group_chat_id>:<uid>:<idx>
    try:
...
async def on_pick(cb: CallbackQuery):
    ...
    # Генерация иллюстрации (оставлено как было; ожидается, что gen.send_illustration вернёт URL иллюстрации или None)
    image_url = await gen.send_illustration(cb.bot, group_chat_id, st["current_situation"], win_answer)

    # Генерация видео через Runway (image-to-video предпочтительно, если есть URL иллюстрации)
    await gen.send_runway_video(
        cb.bot,
        group_chat_id,
        st["current_situation"],
        win_answer,
        reference_image_url=image_url,  # может быть None, тогда будет чистый text-to-video
        duration=5,
        model="gen_4_turbo",
    )
